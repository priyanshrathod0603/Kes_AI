...
import { useUpdateTopic, useChapters } from '@/hooks'
...
export function EditTopicDialog({
  ...
  const handleClose = () => {
    if (!updateTopicMutation.isPending && !isSubmitting) {
      setServerError(null)
      onClose()
    }
  }

  const onSubmit = async (values: EditTopicFormValues) => {
    if (!topic) return
    setServerError(null)
    try {
  ...
  const isPending = isSubmitting || updateTopicMutation.isPending
    ...
    <Dialog open={open} onOpenChange={(isOpen) => (isOpen ? null : handleClose())}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Update topic name or assign it to a different chapter.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
          <div>
            <label htmlFor="editTopicChapter" className="block text-sm font-medium text-foreground mb-1">
              Chapter <span className="text-error-600">*</span>
            </label>
            <select
              id="editTopicChapter"
              disabled={isPending || chaptersLoading}
              {...register('chapterId')}
          ...
          <div>
            <label htmlFor="editTopicName" className="block text-sm font-medium text-foreground mb-1">
              Topic Name <span className="text-error-600">*</span>
            </label>
            <input
              id="editTopicName"
              type="text"
              placeholder="e.g. Conductors vs Insulators"
              disabled={isPending}
              {...register('name')}
